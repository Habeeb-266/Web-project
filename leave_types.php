...
include '../db.php'; // mysql_connect + mysql_select_db ($con)
...
if (isset($_POST['add'])) {
    ...
    if (!empty($type) && $days > 0) {
        mysql_query("INSERT INTO leave_type (type_name, max_days) VALUES ('" . mysql_real_escape_string($type) . "', $days)", $con) or die(mysql_error());
...
if (isset($_POST['edit'])) {
    ...
    $days = (int)$_POST['max_days'];
    if (!empty($type) && $days > 0) {
        mysql_query("UPDATE leave_type SET type_name='" . mysql_real_escape_string($type) . "', max_days=$days WHERE id=$id", $con) or die(mysql_error());
        header("Location: leave_types.php");
        exit();
    }
}
...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    mysql_query("DELETE FROM leave_type WHERE id=$id", $con) or die(mysql_error());
    header("Location: leave_types.php");
    exit();
}

$types = mysql_query("SELECT * FROM leave_type", $con) or die("Query Error: " . mysql_error());
if (!$types) {
    die("Query failed");
}
...
if (mysql_num_rows($types) > 0) {
    ...
    echo "<tr><td colspan='3'>No leave types found in database</td></tr>";
}
?>
</tbody>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    for (var i = 0; i < editButtons.length; i++) {
        editButtons[i].addEventListener('click', function() {
            ...
            document.getElementById('edit-days').value = this.getAttribute('data-days');
            editModal.show();
        });
    }
});
